...
def build_continual_dataloader(args):
    ...
    if args.dataset.startswith('BPIC'):
        path = os.path.join(args.data_path, args.dataset + "_tasks.csv")
        data = pd.read_csv(path, low_memory=False)
        d = Data(args.dataset,
                 LogFile(filename=path, delim=",", header=0, rows=None, time_attr="completeTime", trace_attr="case",
                         activity_attr='event', convert=False))
        d.logfile.keep_attributes(['event', 'role', 'completeTime', 'task'])
        s = Setting(10, "train-test", True, False, 70, 10)
        trainPerc = 0.7
        s.train_percentage = trainPerc * 100
        d.prepare(s)
        dataset_train = d.train.data
        dataset_val = d.test_orig.data
        splited_dataset = split_single_dataset_BPIC15(dataset_train, dataset_val, args)
...
def split_single_dataset_BPIC15(dataset_train, dataset_val, args):
    args.num_tasks = len(set(list(dataset_train['task'])))
    split_datasets = list()
    mask = list()
    
    keyList = ['1', '2', '3', '4', '5']
    indices = {key: [] for key in keyList}
    indices_val = {key: [] for key in keyList}
    
    for ind, i in dataset_train.iterrows():
        indices[str(int(i['task']))] = ind
    
    for ind, i in dataset_val.iterrows():
        indices_val[str(int(i['task']))] = ind
        
    for i in keylist:
        subset_train, subset_val =  Subset(dataset_train, indices[i]), Subset(dataset_val, indices_val[i])
